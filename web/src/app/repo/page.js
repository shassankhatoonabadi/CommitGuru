...
export default function ReposPage() {
  ...
  const pathname = usePathname(); // should be "/repos"
  ...
  async function analyze(repoId) {
    // currently disabled; keep toast so users know
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Repositories <span className="text-muted-foreground font-normal">({meta.total})</span>
          </h1>
      ...
      >
        <RepoControls
          ...
          facets={facets}
          sort={sort} order={order} toggleSort={toggleSort}
          setPage={setPage}
        />
      </div>
